...
import com.github.wzclouds.database.injector.MySqlInjector;
import com.github.wzclouds.database.mybatis.WriteInterceptor;
import com.github.wzclouds.database.mybatis.typehandler.FullLikeTypeHandler;
import com.github.wzclouds.database.mybatis.typehandler.LeftLikeTypeHandler;
import com.github.wzclouds.database.mybatis.typehandler.RightLikeTypeHandler;
import com.github.wzclouds.database.plugins.SchemaInterceptor;
import com.github.wzclouds.database.properties.DatabaseProperties;
import com.github.wzclouds.database.properties.MultiTenantType;
import com.github.wzclouds.uid.service.DisposableWorkerIdAssigner;
...
/**
 * Mybatis 常用重用拦截器，wzclouds.database.multiTenantType=任意模式 都需要实例出来
 * <p>
 * 拦截器执行一定是：
...
public abstract class BaseMybatisConfiguration {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        ...
        if (MultiTenantType.SCHEMA.eq(this.databaseProperties.getMultiTenantType())) {
            ...
            log.info("检测到 wzclouds.database.multiTenantType=SCHEMA，已启用 SCHEMA模式");
        } else if (MultiTenantType.COLUMN.eq(this.databaseProperties.getMultiTenantType())) {
            log.info("检测到 wzclouds.database.multiTenantType=COLUMN，已启用 字段模式");
            // COLUMN 模式 多租户插件
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnExpression("'DEFAULT'.equals('${wzclouds.database.id-type:DEFAULT}') || 'CACHE'.equals('${wzclouds.database.id-type:DEFAULT}')")
